...
def arguments():
    ...
    parser.add_argument("-c", "--configfile", default='/pgcdfga_config/config.yaml',
                        help='The config file to use')
    parser.add_argument("-u", "--ldapuserfile", default='/pgldap/user',
...
def main():
    ...
    '''
    configdata, ldapconfig = config(arguments())
    ...
    LOGGER.debug("PG config %s", pgconn.dsn())

    if pgconn.is_standby():
        LOGGER.debug('Postgres (%s) cluster is standby', pgconn.dsn())
        sys.exit()

    try:
        LOGGER.debug("Processing users %s", configdata['users'])
        process_users(pgconn, configdata['users'], ldapconn)
    except Exception as error:
        LOGGER.exception(str(error))
    try:
        LOGGER.debug("Processing databases %s", configdata['databases'])
        process_databases(pgconn, configdata['databases'])
    except Exception as error:
        LOGGER.exception(str(error))
    try:
        LOGGER.debug("Processing roles %s", configdata['roles'])
        process_roles(pgconn, configdata['roles'])
    except Exception as error:
        LOGGER.exception(str(error))
